...
def get_or_create_session_id():
    ...
    session_id = get_all_cookies_object.get("session_id").value if get_all_cookies_object.get("session_id") else None # type: ignore
    if session_id is None:
        session_id = db.create_session()
        cookies_object = cookies.SimpleCookie()
        cookies_object["session_id"] = session_id # type: ignore
...
# hardcoded undocumented piece of art
def add_to_session(params):
    session_id = get_or_create_session_id()
    _, data = db.get_session(session_id)#vracanje do sada odabranih podataka
    for article_id in params.keys():
